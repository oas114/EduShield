...
| 公文字號 | `DOC_NUM_N` | `[一-龥A-Za-z0-9()（）]{2,12}字第\s*\d{2,10}\s*號` | 臺教中字第1140012345號 |
...
- **有遺漏**：紅色「⚠ 遺漏 N 項，請查看左側紀錄」；為避免壓縮標題列，僅顯示數量而不列出代碼明細。遺漏的詞彙會同時在兩處以**紅色**標示：
  ...
  - 左側底部`#restoreChips`（遮蔽項目膠囊列表）中的對應膠囊變為紅色，且改為**可點擊**（`openMissingTokenFixModal()`）：點擊後跳出小視窗，讓使用者貼上「AI 回覆裡實際出現在該位置的文字」，套用後對 `#restoredOutputView` 做一次精確字串替換（`applyMissingTokenFix()`，比照既有 exact-match 邏輯），成功後該膠囊恢復正常樣式、遺漏計數同步減少。這不是重新設計比對演算法，只是補上「使用者已經自己找到、但沒地方套用」的缺口
...
硬阻斷詞彙、正則規則兩個分頁都支援**線上新增/編輯列**（點「+ 手動新增一列」或直接點擊既有列的欄位），CSV 匯入則保留給批次異動使用。可編輯的對象**包含內建預設規則，不限自訂項目**——`hardBlockKeywords`／`regexRules` 本來就是內建與自訂共用同一個陣列，只是 UI 上不分軒輊。編輯或刪除內建列時，該列會多一個「還原預設」按鈕可單獨復原；已刪除的內建列會出現在表格下方「已從預設清單移除」的清單裡，可點擊加回。這些都是工作階段記憶體狀態，重新整理即回到內建預設值，除非透過下方的匯出/匯入設定檔保留。
...
```
`source` 欄位標示每筆規則的來源，在管理介面以徽章顯示：`內建預設`／`設定檔匯入`／`手動匯入`／`自訂覆蓋`（`customDict` 另有一個內部用的 `ai-session` 標籤，代表當次 AI 擷取的暫存結果，匯出設定檔時會被排除）。內建預設項目另外帶有 `defaultValue`／`defaultPattern`，是回指 `HARD_BLOCK_KEYWORDS_DEFAULT`／`REGEX_RULES_DEFAULT` 對應原始值的穩定連結，編輯多次也不會遺失——用來支援「還原預設」單列復原與「已從預設清單移除」的重新加回功能（見管理介面，「教育隱私保護指南」已改為純概念說明，不再重複顯示規則清單）。
...
正則規則匯入時，`tryCompileRegexRow()` 會做兩層檢查：(1) 是否能通過 `new RegExp()` 編譯；(2) 靜態結構偵測是否存在「群組內外都用不設上限量詞」的巢狀寫法（如 `(a+)+`，最典型的災難性回溯／ReDoS 成因，配合長文字可能讓分頁卡死）。任一項不通過的列都會被單獨列出、跳過，不影響同一批次中其他合法列的匯入；這是簡化過的靜態偵測，只針對最常見的巢狀量詞形狀把關，不是完整的正則 AST 分析。
...
- **匯入設定檔**：開啟檔案選取視窗，選取設定檔後，`extractAutoConfigJson()` 全程只做字串掃描與 `JSON.parse()`（絕不 `eval`／執行檔案內容），逐項驗證後跳出跟 CSV 匯入共用的「合併／完全取代」選擇對話框（`openConfigImportDialog()`），並列出即將匯入的詞庫／硬阻斷／正則規則筆數。**「合併」與「完全取代」的差異在這裡特別重要**：合併是以正則的 pattern 文字／硬阻斷的關鍵字文字本身當作比對鍵，如果你編輯的正是這個欄位（改了 pattern 或關鍵字文字），合併會認不出「這是同一條規則的新版本」，結果變成新舊兩條並存；只有「完全取代」會把該維度清空、直接套用檔案內容，才能正確還原「編輯過或刪除過的內建規則」。來源標籤依模式為「設定檔匯入」或「自訂覆蓋」。
- **轉存為設定檔**：把目前記憶體中四個維度的最新狀態（含所有合併/覆蓋結果，也包含對內建規則的編輯——因為內建與自訂本來就同一個陣列，匯出時不分來源）打包成同樣格式的 JavaScript 檔案供下載，檔名固定為 `edushield.config.js`，可用來分享給同事或在其他電腦重複使用。
...
2. 系統會以 200ms Debounce 即時掃描並以彩色高亮顯示偵測到的敏感詞彙，並在下方「**偵測項目**」膠囊列出所有命中項目，每個膠囊會直接標示類別（如「姓名」「電話」），不需要滑鼠懸浮才看得出來是哪一種。（註：若貼入文字超過 50,000 字元，系統會提示警告，且標記處理可能較慢）。
...
三個頁籤（去識別化／還原／快速遮蔽）皆支援以下快捷鍵，作用於目前所在的頁籤：

| 快捷鍵 | 作用 |
|---|---|
| `Ctrl+Enter` | 執行目前頁籤的主要動作（執行去識別化／執行還原／偵測） |
| `Ctrl+Alt+C` | 複製目前頁籤的結果 |
...
| 還原結果顯示「遺漏 N 項」 | 外部 AI 修改或刪除了 Token 標籤 | 點擊左側對照表中的紅色膠囊，貼上 AI 回覆裡實際出現的文字即可套用修復 |
