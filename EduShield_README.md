...
|------|------|-------------|----------|
| 通道一 | 實體擷取 | 找出人名（含稱謂，如：黃曉美老師、學生：王小明）、廠商機構、地址、專案名稱、帳號 | `[{"type":"PERSON","value":"...","reason":"..."}]` |
| 通道二 | 風險判定 | 判斷是否含特敏資訊（自傷、性侵、家暴等） | `{"critical": true/false, "reason": "..."}` |
...
| 啟動方式 | 直接以瀏覽器開啟 `EduShield.html`，無需安裝套件或伺服器 |
